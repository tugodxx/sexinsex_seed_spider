...
class AAS_Spider(scrapy.Spider):
    name = "AAS"
    def __init__(self, startdate=None, enddate=None, urlprefix='', *args, **kwargs):
        super(AAS_Spider, self).__init__(*args, **kwargs) 
        self._startdate = startdate
        self._enddate = enddate
        self._urlprefix = urlprefix
        self.start_urls = [ urlprefix + 'forum-143-1.html',]
    ...
    def parse(self, response):
        ...
        if firstTime < self._enddate :
            self.crawler.engine.close_spider(self, 'Asia Authorship Seed | 亚洲成人无码原创区, job done!')
